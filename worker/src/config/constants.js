...
export function getRuntimeConfig(env) {
  return {
    ...
    jobMaxProcessingMs: Number(env.JOB_MAX_PROCESSING_MS || 12 * 60 * 1000),
    jobHeartbeatTimeoutMs: Number(env.JOB_HEARTBEAT_TIMEOUT_MS || 4 * 60 * 1000),
    bggDelayMs: Number(env.BGG_DELAY_MS || 1200),
    bggRequestTimeoutMs: Number(env.BGG_REQUEST_TIMEOUT_MS || 15000),
    bggCollectionTimeoutMs: Number(env.BGG_COLLECTION_TIMEOUT_MS || 90 * 1000),
    bggMaxForums: Number(env.BGG_MAX_FORUMS || 6),
    bggMaxThreadsPerForum: Number(env.BGG_MAX_THREADS_PER_FORUM || 6),
    bggMaxCommentsPerThread: Number(env.BGG_MAX_COMMENTS_PER_THREAD || 6),
